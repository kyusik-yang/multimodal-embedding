...
The pilot produces embeddings from three strategies (text, audio, multimodal) for the same Shorts. Comparing them tells us whether multimodal adds value over simpler approaches.
...
for s1, s2 in strategy_pairs:
    ...
    print(f"  {s1:12} vs {s2:12}: {mean_dist:.4f}")
```

::: {.callout-note}
## How to interpret the comparison
- **Large distance** (> 0.05) between audio and multimodal: the video channel adds meaningful information. Multimodal is worth the extra cost.
- **Small distance** (< 0.02): audio captures most of the relevant variation. Use Audio + Text to save money.
:::

## Pilot checklist

After running the pilot, answer these questions before scaling up:

- [ ] Does multimodal add value over audio-only? (distance > 0.05)
- [ ] Do same-party politicians cluster together in UMAP?
- [ ] Is the File API upload speed acceptable for batch processing?
- [ ] Which cost tier fits the budget? (Audio+Text ~$200 vs Multimodal ~$1,300 for 30K)
- [ ] Are there any Shorts that fail to embed? (format issues, duration limits)
